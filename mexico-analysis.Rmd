...
```{r areawithtransects, fig.cap="The survey area with transect lines.", fig.height=4}
...
The following code generates a plot of the fitted detection function and quantile-quantile plot:

``` {r hr-detfct, fig.cap="Plot of the fitted detection function (left) and goodness of fit plot (right) for the hazard-rate model.", fig.width=9, fig.height=6, results="hide"}
...
```{r visgam1, fig.cap="Plot of the spatial smooth in `dsm.xy`, values are relative abundances. White/yellow indicates high values, red low indicates low values."}
...
```{r dsm-xy-depth-depth, fig.cap="Plot of the smooth of depth in `dsm.xy.depth`. This \"hockey stick\" shaped smooth indicates that there is not much information in depth after 500m."}
...
```{r visgam5, fig.cap="Plot of the spatial smooth in `dsm.est.xy`."}
...
```{r dsm.xy-check, fig.cap="Diagnostic plots for `dsm.xy`.", fig.width=6, fig.height=6}
gam.check(dsm.xy)
```
...
``` {r dsm.xy.tweedie-check, fig.cap="Diagnostic plots for `dsm.xy.tweedie`.", fig.width=6, fig.height=6}
...
``` {r dsm.xy.tweedie-rqcheck, fig.cap="Randomised quantile residual diagnostic plots for `dsm.xy.tweedie`.", fig.width=6, fig.height=6}
...
```{r dsm.xy-preds, fig.cap="Predicted density surface for `dsm.xy`."}
...
```{r dsm.xy.depth-preds, fig.cap="Predicted density surface for `dsm.xy.depth`."}
...
We can also make a plot of the CVs using the following code.

```{r dsm.xyvarplot, fig.cap="Plot of the coefficient of variation for the study area with transect lines and observations overlaid. Note the increase in CV away from the transect lines."}
